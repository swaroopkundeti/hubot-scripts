...
jenkinsBuild = (msg, buildWithEmptyParameters) ->
    ...
    path = if params then "#{url}/job/(.*)/job/#{job}/buildWithParameters?#{params}" else "#{url}/job/(.*)/job/#{job}/#{command}"
    ...
    req.post() (err, res, body) ->
        if err
          ...
          msg.reply "(#{res.statusCode}) Build started for #{job} #{url}/job/(.*)/job/#{job}"
    ...
    req.get() (err, res, body) ->
        if err
          ...
          try
            ...
            path = "#{url}/job/(.*)/job/#{job}/#{content.lastBuild.number}/api/json"
            req = msg.http(path)
            if process.env.HUBOT_JENKINS_AUTH
              auth = new Buffer(process.env.HUBOT_JENKINS_AUTH).toString('base64')
              req.headers Authorization: "Basic #{auth}"
...
jenkinsLast = (msg) ->
    ...
    path = "#{url}/job/(.*)/job/#{job}/lastBuild/api/json"
